...
桌面端 AI 助手应用，基于 Electron + Vue 3 + TypeScript 构建，集成智能对话、知识库管理、代码沙箱执行和文档生成等功能。
...
- **多格式导出**：支持将对话内容导出为 Markdown、Word、PDF、TXT
- **PPT 智能生成**：基于模板自动生成演示文稿
  - 自动解析 Markdown 内容生成幻灯片结构
  - 支持自定义模板格式和布局
  - 智能匹配内容到模板占位符
  - 支持图表、表格、列表等复杂元素
- **模板管理**：上传和管理 PPT 模板，支持多种布局和样式
...
- **智谱 AI 集成**：支持调用智谱 AI 视频生成接口
- **参数配置**：可设置分辨率（横版/竖版）、质量模式、是否带音频
- **任务管理**：异步任务状态追踪，支持轮询获取生成结果
- **预览与下载**：生成完成后可直接预览和下载视频

### 语音输入

- **阿里云语音识别**：集成阿里云 NLS 语音听写服务
- **实时转写**：边说边转文字，支持中文语音识别
- **一键输入**：语音转换后一键填入消息框
...
1. 在对话中要求 AI 生成 PPT 内容（使用 Markdown 格式）
2. 点击导出按钮选择 PPT 格式
3. 选择模板和布局配置
4. 下载生成的演示文稿
