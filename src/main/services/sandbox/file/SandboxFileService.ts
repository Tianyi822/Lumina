...
import type { FileWriteRequest, FileWriteResult } from '@shared/types/sandbox'
import { sandboxService } from '../SandboxService'
import { getDockerService } from '../docker/DockerService'
import {
  getSandboxFileTempRoot,
  isPathInTempRoot,
...
export class SandboxFileService {
  ...
  async writeProjectFiles(
    ...
    if (!sandbox) {
      return { success: false, writtenCount: 0, error: '沙箱不存在' }
    }

    const containerId = sandbox.primaryContainerId || sandbox.containerIds[0]
    if (!containerId) {
      return { success: false, writtenCount: 0, error: '沙箱没有关联容器' }
    }
    ...
    try {
      await fs.mkdir(getSandboxFileTempRoot(), { recursive: true })
      tempDir = await fs.mkdtemp(join(getSandboxFileTempRoot(), `${sandboxId}-`))
