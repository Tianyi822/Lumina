...
export function decodeBase64Url(value: string, expectedBytes?: number): Uint8Array {
  if (!/^[A-Za-z0-9_-]*$/.test(value)) {
    throw new Error(t('notifications.sync.base64UrlFormatInvalid'))
  }
  const decoded = new Uint8Array(Buffer.from(value, 'base64url'))
  if (encodeBase64Url(decoded) !== value) {
    throw new Error(t('notifications.sync.base64UrlCanonicalInvalid'))
  }
  if (expectedBytes !== undefined && decoded.length !== expectedBytes) {
    throw new Error(
      t('notifications.sync.base64UrlLengthInvalid', {
