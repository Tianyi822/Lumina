...
import type { DockerCheckResult, PlatformType, SandboxResult } from '@shared/types/sandbox'
import { execAsync, createErrorResult } from './shared'

/**
 * 注册 Docker 基础处理器
 */
export function registerSandboxDockerHandlers(): void {
  ...
  ipcMain.handle('sandbox:openExternal', async (_event, url: string): Promise<SandboxResult> => {
    try {
