...
test('AI 建议按钮具备明确 aria-label', async () => {
  ...
  assert.match(source, /i18n\.t\('writer\.suggestions\.ariaLabel'\)/)
  assert.match(source, /i18n\.t\('writer\.suggestions\.acceptAllAria'\)/)
  assert.match(source, /i18n\.t\('writer\.suggestions\.rejectAllAria'\)/)
  assert.match(source, /i18n\.t\('writer\.suggestions\.acceptAria'\)/)
  assert.match(source, /i18n\.t\('writer\.suggestions\.rejectAria'\)/)
...
test('Slash Menu 使用 listbox 与 option 语义', async () => {
  ...
  assert.match(source, /aria-label=\{t\('writer\.slashMenu\.ariaLabel'\)\}/)
  assert.match(source, /role="option"/)
})

test('标题输入与保存状态具备 label 与 aria-live', async () => {
  const source = await readFile(new URL('./WriterEditor.tsx', import.meta.url), 'utf8')
  assert.match(source, /aria-label=\{t\('writer\.editor\.titleAriaLabel'\)\}/)
...
test('Bubble Menu 暴露改写与续写动作 aria-label', async () => {
  ...
  assert.match(source, /aria-label=\{t\('writer\.bubbleMenu\.aiRewriteAria'\)\}/)
  assert.match(source, /aria-label=\{t\('writer\.bubbleMenu\.aiContinueAria'\)\}/)
