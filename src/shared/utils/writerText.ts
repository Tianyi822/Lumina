 ...
 * 写作文本规范化与稳定哈希。
 *
 * 实现选择：使用纯 JS SHA-256，避免 Node `crypto` 被渲染进程直接 import。
 * 同一实现同时服务 Electron 主进程、渲染进程与 node:test，保证哈希一致。
 */
...
export function hashWriterText(text: string): string {
  return sha256Hex(normalizeWriterText(text))
}

/** 纯 JS SHA-256（同步），UTF-8 编码后摘要为小写 hex */
...
function sha256(message: Uint8Array): Uint8Array {
  ...
  // 以大端写入 64-bit 长度（写作文本远小于 2^32 字节，高 32 位为 0）
